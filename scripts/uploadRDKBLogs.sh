...
useDirectUploadToUpstream()
{
    ...
    do
        ...
        echo_t "Executing: curl -X POST -F files=@$UploadFile $UploadHttpLink"
        
        # Run curl and capture both stdout and return code
        # Add explicit timeout and max-time
        curl -X POST -F "files=@$UploadFile" --connect-timeout 30 --max-time $UPSTREAM_TIMEOUT "$UploadHttpLink" > /tmp/curl_stdout.txt 2>"$CURL_LOG"
        ret=$?
